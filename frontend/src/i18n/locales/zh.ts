export default {
  ...
  admin: {
    ...
    groups: {
      ...
      copyAccounts: {
        title: '从分组复制资源',
        tooltip: '选择一个或多个相同平台的分组，创建后会自动将这些分组的所有资源实例绑定到新分组（去重）。',
        tooltipEdit: '选择一个或多个相同平台的分组，保存后当前分组的资源实例会被替换为这些分组的资源实例（去重）。',
        selectPlaceholder: '选择分组以复制其资源实例...',
        hint: '可选多个分组，资源实例会自动去重',
        hintEdit: '⚠️ 注意：这会替换当前分组的所有资源实例绑定'
      },
      modelRouting: {
        title: '模型路由配置',
        tooltip:
          '配置特定模型请求优先路由到指定资源实例。支持通配符匹配，如 claude-opus-* 匹配所有 opus 模型。',
        enabled: '已启用',
        disabled: '已禁用',
        ...
        accounts: '优先实例',
        selectAccounts: '选择实例',
        noAccounts: '此分组暂无资源实例',
        loadingAccounts: '加载实例中...',
        removeRule: '删除规则',
        noRules: '暂无路由规则',
        noRulesHint: '添加路由规则以将特定模型请求优先路由到指定资源实例',
        searchAccountPlaceholder: '搜索资源实例...',
        accountsHint: '选择此模型模式优先使用的资源实例'
      },
      mcpXml: {
        title: 'MCP XML 协议注入',
    ...
    riskControl: {
      ...
      autoBan: '自动封禁用户',
      autoBanHint: '命中次数达到阈值后将禁用用户账户、刷新认证缓存并发送封禁通知邮件。',
      banThreshold: '封禁触发次数',
    ...
    channelMonitor: {
      ...
      form: {
        ...
        selectKeyHint: '仅显示当前用户下处于「启用」状态且未过期的 Key。',
